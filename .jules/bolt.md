...
## 2024-11-21 - Optimize WebSocket broadcast scheduling overhead
**Learning:** In high-frequency `asyncio` networking applications (like WebSocket broadcasting), `asyncio.run_coroutine_threadsafe` and `await asyncio.gather(...)` add significant intermediate coroutine scheduling overhead for fire-and-forget messages.
**Action:** Replace `asyncio.run_coroutine_threadsafe` and `gather` with `loop.call_soon_threadsafe(func)` (where `func` calls `loop.create_task()` directly and tasks are strongly referenced to avoid garbage collection). This removes the intermediate coroutine scheduling overhead, significantly improving throughput (up to 4x faster).
