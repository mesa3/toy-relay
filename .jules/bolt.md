...
## 2024-11-20 - Fast WebSocket Broadcasting
**Learning:** In high-frequency `asyncio` networking applications (like WebSocket broadcasting), using `asyncio.run_coroutine_threadsafe` and `await asyncio.gather(...)` introduces intermediate coroutine scheduling overhead, slowing down the loop.
**Action:** Replace `asyncio.run_coroutine_threadsafe` and `await asyncio.gather(...)` with `loop.call_soon_threadsafe(func)` (where `func` calls `loop.create_task()` directly). This significantly improves throughput for fire-and-forget messages (by ~30%). Maintain strong references to tasks to avoid garbage collection.
