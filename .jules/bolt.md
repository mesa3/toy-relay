...
**Learning:** In high-frequency `asyncio` networking applications (like WebSocket broadcasting), using `asyncio.run_coroutine_threadsafe` and `await asyncio.gather(...)` introduces intermediate coroutine scheduling overhead for fire-and-forget messages.
**Action:** Replace them with `loop.call_soon_threadsafe(func)` (where `func` iterates over clients and calls `loop.create_task()` directly) to remove this overhead and significantly improve throughput. Always maintain strong references to tasks (e.g. `self._bg_tasks = set()`) when using `create_task()` to prevent garbage collection mid-execution.
