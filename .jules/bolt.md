...
## 2024-11-20 - Replace `run_coroutine_threadsafe` and `gather` with `call_soon_threadsafe`
**Learning:** In high-frequency `asyncio` networking applications (like WebSocket broadcasting), replacing `asyncio.run_coroutine_threadsafe` and `await asyncio.gather(...)` with `loop.call_soon_threadsafe(func)` (where `func` calls `loop.create_task()` directly) removes intermediate coroutine scheduling overhead, significantly improving throughput for fire-and-forget messages. Benchmarks show this approach is roughly 40-50% faster.
**Action:** When sending high-frequency, non-blocking (fire-and-forget) messages via asyncio to multiple clients, avoid `run_coroutine_threadsafe` and `gather`. Instead, schedule a synchronous wrapper function via `call_soon_threadsafe` that directly spins off tasks for each client using `loop.create_task()`, making sure to track the tasks and catch exceptions properly.
