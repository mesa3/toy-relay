...
## 2024-11-21 - Fire-and-forget Async Task Optimization
...
**Action:** When creating high-throughput async pipelines for fire-and-forget sending, use `loop.call_soon_threadsafe(schedule_func)` where `schedule_func` creates tasks using `loop.create_task()`, avoiding `asyncio.gather`. Also, always maintain strong references to the tasks using a set (`self._bg_tasks.add(task)`) and remove them with a callback (`task.add_done_callback(self._bg_tasks.discard)`) to prevent premature garbage collection mid-execution.
