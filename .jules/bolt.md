...
## 2024-11-20 - Asyncio Broadcast Overhead
**Learning:** In high-frequency `asyncio` networking applications (like WebSocket broadcasting), `asyncio.run_coroutine_threadsafe` and `await asyncio.gather(...)` introduce measurable intermediate coroutine scheduling overhead, particularly for fire-and-forget messaging where results are not needed.
**Action:** Replace them with `loop.call_soon_threadsafe(func)` where `func` directly calls `loop.create_task()`. Be sure to maintain a strong reference to created tasks (e.g., using a tracked `set()` and `.add_done_callback()`) to prevent garbage collection during execution.
