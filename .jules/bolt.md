...
## 2024-11-21 - Fire-and-Forget WebSocket Broadcasting Overhead
**Learning:** In high-frequency `asyncio` networking applications (like WebSocket broadcasting), using `asyncio.run_coroutine_threadsafe` and `await asyncio.gather(...)` with `return_exceptions=True` for fire-and-forget messages incurs significant overhead. This is because `run_coroutine_threadsafe` wraps the coroutine in a `concurrent.futures.Future` to block and pass results across threads, and `gather` incurs overhead tracking multiple results that are ultimately discarded.
**Action:** Replace `asyncio.run_coroutine_threadsafe` and `await asyncio.gather(...)` with `loop.call_soon_threadsafe(func)`, where `func` directly calls `loop.create_task()`. Ensure tasks are strongly referenced to prevent garbage collection mid-execution by storing them in a set (e.g., `self._bg_tasks.add(task)`) and clearing them using `task.add_done_callback(self._bg_tasks.discard)`. Handle exceptions internally within the scheduled task (`except Exception: pass`) to replicate the safety of `gather(..., return_exceptions=True)`.
