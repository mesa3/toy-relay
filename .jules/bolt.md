...
## 2024-11-20 - Optimize asyncio fire-and-forget broadcast overhead
**Learning:** In high-frequency asyncio networking applications (like WebSocket broadcasting), `asyncio.run_coroutine_threadsafe` combined with `await asyncio.gather(...)` introduces significant coroutine scheduling and state-management overhead for simple fire-and-forget operations.
**Action:** Replace this pattern with `loop.call_soon_threadsafe(func)`, where `func` directly calls `loop.create_task()`. Ensure tasks are tracked in a set to avoid garbage collection (`task.add_done_callback(set.discard)`), and explicitly catch exceptions inside the task to prevent 'Task exception was never retrieved' warnings. This structural change significantly improves throughput by bypassing full coroutine evaluation overhead.
