...
## 2024-11-20 - WebSockets Task Creation Overhead
**Learning:** `asyncio.run_coroutine_threadsafe` and `asyncio.gather` combined add significant overhead when broadcasting messages to websockets because of the intermediate coroutine scheduling step.
**Action:** Use `loop.call_soon_threadsafe(func)` which directly creates `loop.create_task` tasks for each websocket message. This skips the extra layer of coroutines and significantly speeds up fire-and-forget broadcasting throughput.
