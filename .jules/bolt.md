...
## 2024-11-21 - Optimize Asyncio Tasks in Hot Broadcasting Loops
**Learning:** In high-frequency network applications (like WebSocket broadcasting), combining `asyncio.run_coroutine_threadsafe` with `await asyncio.gather(...)` introduces significant overhead due to intermediate coroutine creation and scheduling. Directly using `loop.call_soon_threadsafe()` to run a synchronous method that spins up fire-and-forget tasks using `loop.create_task()` removes this bottleneck and dramatically improves throughput (by up to ~37%).
**Action:** When broadcasting fire-and-forget messages to multiple clients in a hot loop, replace `run_coroutine_threadsafe` and `gather` with `call_soon_threadsafe(func)`, where `func` directly calls `loop.create_task()` for each client. Always keep strong references to the created tasks in a `set` (and use `add_done_callback(set.discard)`) to avoid premature garbage collection, and wrap the send operation in a try-except block to safely discard uncaught task exceptions.
