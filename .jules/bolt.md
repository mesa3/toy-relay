...
**Learning:** In high-frequency `asyncio` networking applications (like WebSocket broadcasting), using `asyncio.run_coroutine_threadsafe` and `await asyncio.gather(...)` to send messages creates unnecessary coroutine scheduling and object creation overhead for simple fire-and-forget operations.
**Action:** Replace `run_coroutine_threadsafe` and `gather` with `loop.call_soon_threadsafe(func)`, where `func` directly calls `loop.create_task()`. Wrap the async call in a try-except block to discard exceptions safely, and maintain strong references to tasks (e.g., using a `self._bg_tasks` set) to prevent garbage collection mid-execution.
