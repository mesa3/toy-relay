...
## 2024-11-21 - Optimize WebSocket Broadcasting Throughput
**Learning:** In high-frequency `asyncio` networking applications (like WebSocket broadcasting), `asyncio.run_coroutine_threadsafe` and `await asyncio.gather(...)` add unnecessary intermediate coroutine scheduling overhead for fire-and-forget messages.
**Action:** Replace them with `loop.call_soon_threadsafe(func)`, where `func` iterates over the clients and creates individual background tasks using `loop.create_task()`. Ensure you use a strong reference tracker (`_bg_tasks` set) and wrap `send` in a try-except block to discard exceptions safely. This reduces overhead and speeds up the broadcasting hot path.
