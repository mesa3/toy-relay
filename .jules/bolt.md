...
## 2024-11-21 - Optimize WebSocket Broadcasting in Hot Loop
**Learning:** In high-frequency `asyncio` networking applications (like WebSocket broadcasting), `asyncio.run_coroutine_threadsafe` and `await asyncio.gather(...)` introduce intermediate coroutine scheduling overhead. For fire-and-forget messaging, this overhead is noticeable and limits throughput. Additionally, when using `loop.create_task()` directly as an optimization, garbage collection can destroy tasks mid-execution if strong references aren't maintained, and unhandled exceptions can trigger 'Task exception was never retrieved' warnings.
**Action:** Replace `run_coroutine_threadsafe` and `gather` with `loop.call_soon_threadsafe(func)` where `func` directly calls `loop.create_task()`. Maintain strong references to tasks by adding them to a dedicated set (e.g., `self._bg_tasks = set()`) upon creation and removing them via `task.add_done_callback()`. Always wrap the target async call (like `client.send`) in a try-except block to safely discard exceptions and prevent warnings.
