...
## 2024-11-20 - WebSocket Broadcast Fire-and-Forget Optimization
**Learning:** In high-frequency `asyncio` networking applications (like WebSocket broadcasting), using `asyncio.run_coroutine_threadsafe` and `await asyncio.gather(...)` introduces intermediate coroutine scheduling overhead. For fire-and-forget messages, this is unnecessary.
**Action:** Replace `asyncio.run_coroutine_threadsafe` and `await asyncio.gather(...)` with `loop.call_soon_threadsafe(func)`, where `func` directly calls `loop.create_task()`. Ensure tasks are tracked (e.g., using a set and `add_done_callback`) to prevent garbage collection mid-execution. This structural change significantly improves throughput.
