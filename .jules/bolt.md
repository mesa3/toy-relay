...
## 2024-11-20 - Eliminate Future/Lock Overhead in Asyncio Fire-and-Forget Loops
**Learning:** In high-frequency loop boundaries where a synchronous thread calls an asynchronous event loop (e.g. WebSocket broadcasting), using `asyncio.run_coroutine_threadsafe(coro)` creates massive overhead. It instantiates a thread-safe `concurrent.futures.Future`, which uses locks internally, blocks on scheduling, and involves heavy synchronization. When combined with `asyncio.gather(*[...], return_exceptions=True)`, the loop is artificially stalled waiting for the slowest client.
**Action:** For fire-and-forget network messages, replace `run_coroutine_threadsafe(gather())` with `loop.call_soon_threadsafe(loop.create_task(...))`. This completely bypasses Future creation and lock resolution, scheduling the sends natively in the event loop. Always maintain a tracking set (e.g. `self._bg_tasks = set()`) and attach a done callback (`task.add_done_callback(self._bg_tasks.discard)`) to prevent the garbage collector from destroying the unawaited tasks mid-execution.
