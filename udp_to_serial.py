...
class TCodeWSServer:
    ...
    def broadcast(self, message):
        ...
        ⚡ Optimized: Replaced asyncio.run_coroutine_threadsafe and await asyncio.gather
        with loop.call_soon_threadsafe and direct create_task calls. This removes
        intermediate coroutine scheduling overhead, significantly improving throughput
        for fire-and-forget messages.
        """
        if not self.running or not self.clients or not self.loop:
            return

        self.loop.call_soon_threadsafe(self._do_broadcast, message)
