...
class TCodeWSServer:
    ...
    def _fire_and_forget(self, message):
        if not self.clients:
            return
    ...
    def broadcast(self, message):
        ...
        with direct loop.call_soon_threadsafe and create_task() for lower overhead.
        """
        if not self.running or not self.clients or not self.loop:
            return

        self.loop.call_soon_threadsafe(self._fire_and_forget, message)
