...
class TCodeWSServer:
    ...
    def _broadcast_internal(self, message):
        ...
        for client in self.clients:
            create_task(self._safe_send(client, message))

    def broadcast(self, message):
        """Broadcasts message to all connected clients.
        ⚡ Optimized: Avoids intermediate coroutine and gather overhead by using
        call_soon_threadsafe and creating tasks directly on the event loop.
        """
        if not self.running or not self.clients or not self.loop:
            return

        self.loop.call_soon_threadsafe(self._broadcast_internal, message)
