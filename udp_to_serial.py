...
class TCodeWSServer:
    ...
    def broadcast(self, message):
        ...
        ⚡ Optimized: Replaced asyncio.run_coroutine_threadsafe and gather
        with direct loop.call_soon_threadsafe and loop.create_task to
        significantly reduce coroutine scheduling overhead.
        """
        if not self.running or not self.clients or not self.loop:
            return

        self.loop.call_soon_threadsafe(self._broadcast_sync, message)
