...
class TCodeWSServer:
    ...
    def _schedule_broadcast(self, message):
        for client in self.clients:
            task = self.loop.create_task(self._safe_send(client, message))
            self._bg_tasks.add(task)
            task.add_done_callback(self._bg_tasks.discard)

    def broadcast(self, message):
        """Broadcasts message to all connected clients.
        ⚡ Optimized: Replaced `asyncio.gather` and `run_coroutine_threadsafe`
        with direct `create_task` calls scheduled via `call_soon_threadsafe`.
        This removes intermediate coroutine scheduling overhead and significantly
        improves throughput for fire-and-forget messages.
        """
