...
class TCodeWSServer:
    ...
    def _fire_broadcast(self, message):
    ...
    def broadcast(self, message):
        ...
        ⚡ Bolt: Uses direct loop.create_task instead of asyncio.gather to avoid overhead.
        """
        if not self.running or not self.clients or not self.loop:
            return

        self.loop.call_soon_threadsafe(self._fire_broadcast, message)
