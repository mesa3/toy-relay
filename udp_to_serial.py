...
class TCodeWSServer:
    ...
    def broadcast(self, message):
        ...
        with `loop.call_soon_threadsafe(create_task)` loop to drastically reduce
        intermediate coroutine scheduling and Future/Lock resolution overhead
        for fire-and-forget messages. Tasks are stored in `self._bg_tasks`
        to prevent garbage collection mid-execution.
        """
        if not self.running or not self.clients or not self.loop:
            return

        self.loop.call_soon_threadsafe(self._broadcast_to_clients, message)
