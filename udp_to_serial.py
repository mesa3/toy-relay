...
class TCodeWSServer:
    ...
    def _queue_broadcast(self, message):
    ...
    def broadcast(self, message):
        ...
        ⚡ Optimized: Replaced run_coroutine_threadsafe and gather with call_soon_threadsafe
        and direct task creation to eliminate coroutine scheduling overhead.
        """
        if not self.running or not self.clients or not self.loop:
            return

        self.loop.call_soon_threadsafe(self._queue_broadcast, message)
